        ...
        <div className="grid gap-6 lg:grid-cols-3 mb-6">
          ...
          <Card className="bg-card border-border lg:col-span-2">
            <CardHeader className="pb-2">
              <CardTitle className="text-base font-medium">Sensor Readings (24h)</CardTitle>
              <p className="text-xs text-muted-foreground">Temperature · Vibration · Pressure</p>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={sensorData} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                  <defs>
                  ...
                  <Area type="monotone" dataKey="temp" name="Temp (°C)" stroke="#6366f1" fill="url(#gradTemp)" strokeWidth={2} />
                  <Area type="monotone" dataKey="vibration" name="Vibration" stroke="#10b981" fill="url(#gradVib)" strokeWidth={2} />
                  <Area type="monotone" dataKey="pressure" name="Pressure" stroke="#f59e0b" fill="url(#gradPres)" strokeWidth={2} />
                </AreaChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
        ...
        <div className="grid gap-6 lg:grid-cols-2 mb-6">
          ...
          <Card className="bg-card border-border">
            <CardHeader className="pb-2">
              ...
              <p className="text-xs text-muted-foreground">Current accuracy per model (%)</p>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={modelAccuracy} layout="vertical" margin={{ top: 0, right: 16, left: 4, bottom: 0 }}>
          ...
          <Card className="bg-card border-border">
            <CardHeader className="pb-2">
              <CardTitle className="text-base font-medium">Weekly Predictions</CardTitle>
              <p className="text-xs text-muted-foreground">Prediction volume vs errors this week</p>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={220}>
                <LineChart data={weeklyPredictions} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                  ...
                  />
                  <Legend wrapperStyle={{ fontSize: "11px" }} />
                  <Line yAxisId="left" type="monotone" dataKey="predictions" name="Predictions" stroke="#6366f1" strokeWidth={2} dot={false} />
                  <Line yAxisId="right" type="monotone" dataKey="errors" name="Errors" stroke="#ef4444" strokeWidth={2} dot={false} strokeDasharray="4 2" />
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    {["Workspace", "Models", "Nodes", "Data Points (M)", "Health"].map((h) => (
                      <th
                        key={h}
                ...
                <tbody className="divide-y divide-border">
                  {workspaceUsage.map((w) => (
                    <tr key={w.workspace} className="hover:bg-accent/30 transition-colors">
                      <td className="px-4 py-3 text-sm font-medium text-foreground">{w.workspace}</td>
                      <td className="px-4 py-3 text-sm text-muted-foreground">{w.models}</td>
                      <td className="px-4 py-3 text-sm text-muted-foreground">{w.nodes}</td>
                      <td className="px-4 py-3 text-sm text-muted-foreground">{w.dataPoints}M</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div className="flex-1 h-1.5 rounded-full bg-muted overflow-hidden">
                            <div
                              className="h-full rounded-full bg-emerald-500"
                              style={{ width: `${Math.round((w.models / 60) * 100)}%` }}
                            />
                          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <Card className="bg-card border-border">
            <CardContent className="pt-6">
              ...
              <p className="text-xs text-muted-foreground mt-1">67% of all models</p>
            </CardContent>
          </Card>
          <Card className="bg-card border-border">
            <CardContent className="pt-6">
              ...
              <div className="mt-3 h-1.5 rounded-full bg-muted overflow-hidden">
                <div className="h-full bg-muted-foreground rounded-full" style={{ width: "25%" }} />
              </div>
              <p className="text-xs text-muted-foreground mt-1">25% of all models</p>
            </CardContent>
          </Card>
          <Card className="bg-card border-border">
            <CardContent className="pt-6">
              ...
              <p className="text-xs text-muted-foreground mt-1">8% of all models</p>
            </CardContent>
          </Card>
        </div>
      </div>
